...
// 미리보기 구현
const handlerFile = (e) => {
  // 현재 파일이 이미지 파일이면, 썸네일이 나타나게 할 것임.

  // 선택된 파일의 확장자를 가져오는 겁니다. (jpg인지? png인지? hwp?)
  const fileInfo = e.target.files;
  const fileInfoSplit = fileInfo[0].name.split('.');
  const fileExtenstion = fileInfoSplit[1].toLowerCase(); // 확장자명을 소문자 통일

  // 만약 선택한 파일이 이미지이면 썸네일을 생성하는 것이다.
  if (['jpg', 'gif', 'png'].includes(fileExtenstion)) {
...
const downloadFile = () => {
  ...
  axios.post('/api/support/noticeDownload.do', param, { responseType: 'blob' }).then((res) => {
    ...
    // <a href='link' download='detail.value.fileName'></a>
